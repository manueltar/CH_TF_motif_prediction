...
collect = function(option_list)
{
  ...
  ALL_df_broken2<-as.data.frame(cSplit(ALL_df_broken1, splitCols= "query_region_2", sep = ";", direction = "wide", fixed = TRUE, drop=F), stringsAsFactors=F)
  ...
  colnames(ALL_df_broken2)[which(colnames(ALL_df_broken2) == 'query_region_1')]<-'cell_type'
  colnames(ALL_df_broken2)[which(colnames(ALL_df_broken2) == 'query_region_2_2')]<-'CHIP_factor'
  
  indx.dep<-which(colnames(ALL_df_broken2)%in%c('query_region_2','query_region_2_1'))
  ...
  {
    ALL_df_broken2<-ALL_df_broken2[,-indx.dep]
