...
data_wrangling_and_intersect = function(option_list)
{
  ...
  file_list_sel$ensembl_gene_id<-gsub("\\.out\\.gz","",file_list_sel$file)
  ...
  write.table(Result, 
              file=paste("TF_motifs_supported_by_chipseq",".tsv",sep=""), 
...
merge_with_final_table = function(option_list)
{
  ...
  write.table(Final_table_TF_motif_prediction, 
              file=paste("Final_table_TF_motif_prediction_with_chipseq_support",".tsv",sep=""), 
              row.names = F, quote=F, sep="\t")
